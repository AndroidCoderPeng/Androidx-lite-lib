...
public class SteeringWheelController extends View implements View.OnTouchListener {
    ...
    //画布中心x
    private float canvasCenterX = 0f;

    //画布中心y
    private float canvasCenterY = 0f;
    ...
    private boolean centerTurn = false;

    public SteeringWheelController(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        TypedArray type = context.obtainStyledAttributes(attrs, R.styleable.SteeringWheelController);
        int viewBackground = type.getColor(R.styleable.SteeringWheelController_controller_backgroundColor, Color.BLACK);
        borderColor = type.getColor(R.styleable.SteeringWheelController_controller_borderColor, Color.RED);
        int temp = type.getInt(R.styleable.SteeringWheelController_controller_outerCircleDiameter, 120);
        outerCircleDiameter = FloatHub.dp2px(context, temp);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        canvasCenterX = (w >> 1);
        canvasCenterY = (h >> 1);
        centerSwitchOval = new RectF(
                canvasCenterX - (float) outerCircleDiameter / 15,
                canvasCenterY - (float) outerCircleDiameter / 15 + 2,
                canvasCenterX + (float) outerCircleDiameter / 15,
                canvasCenterY + (float) outerCircleDiameter / 15 + 2
        );
    ...
    private int measureWidth(int defaultWidth, int measureSpec) {
        ...
        if (specMode == MeasureSpec.AT_MOST) {
            ...
            width = Math.min(defaultWidth, specSize);
        }
        return width;
    }
    ...
    private int measureHeight(int defaultHeight, int measureSpec) {
        ...
        if (specMode == MeasureSpec.AT_MOST) {
            ...
            height = Math.min(defaultHeight, specSize);
        }
        return height;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawLine(
                canvasCenterX, canvasCenterY - (float) outerCircleDiameter / 15 - 2,
                canvasCenterX, canvasCenterY - (float) outerCircleDiameter / 15 + 15,
                centerSwitchPaint
        );
        ...
        if (leftTurn) {
            ...
            canvas.drawArc(outerCircleRectF, (90 * 2 - 45), 90f, false, leftDirectionPaint);
        } else {
            leftDirectionPaint.setColor(borderColor);
        }
        ...
        if (topTurn) {
            ...
            canvas.drawArc(outerCircleRectF, (90 * 3 - 45), 90f, false, topDirectionPaint);
        } else {
            topDirectionPaint.setColor(borderColor);
        }
